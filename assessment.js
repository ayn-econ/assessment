...
function removeAllChildren(element) {
    while (element.firstChild) {
        // 子どもの要素がある限り削除　これを行わないと、診断を何度も押すと何度も同じ文章が表示されてしまう
...
assessmentButton.onclick = () => { // アロー関数という書き方。ES6であればこの形で簡単に書ける。
    ...
    removeAllChildren(resultDevided); // removeAllChildren関数を作成したことで、ここの記述をシンプルにできる
...
function assessment(userName) {
    ...
    const index = sumOfCharCode % answers.length; // indexは添え字の意味。constで設定する変数は更新不可
    let result = answers[index]; // letは変更可能な変数
...
// Enter をクリックする事で診断ができる機能の実装
userNameInput.onkeydown = event => {
    if (event.key === 'Enter') {
        assessmentButton.onclick(); // プロパティonclickに実装されている関数を呼び出す記述。20行目で既に機能を作っているので、改めてコードを1から書き加えずにこの書き方をすればよい。
    }
}
